...
@web_router.post("/quantity-product-decrease")
...
@web_router.post("/quantity-product-increase")
...
@web_router.post("/quantity-product-delete")
...
def success_payment(request: Request, user=Depends(get_user_web)):
    context = {"request": request, "title": "success payment", "user": user}
...
def web_login(request: Request, email: str = Form(None), password: str = Form(None), user=Depends(get_user_web)):
    if user:
        redirect_url = request.url_for("index")
        response = RedirectResponse(redirect_url, status_code=status.HTTP_303_SEE_OTHER)
        response_with_cookies = set_cookies_web(user, response)
...
def web_logout(request: Request):
    context = {"request": request, "products": get_all_products(50, 0, ""), "title": "Main page", "user": None}
    response = templates.TemplateResponse("index.html", context=context)
